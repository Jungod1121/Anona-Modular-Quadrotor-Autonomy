/**
 * Path F — MIGHTY-inspired Hermite spline planner for drone_ws plant contract.
...
class FastPlanningNode : public rclcpp::Node {
public:
  FastPlanningNode() : Node("fast_planning_node") {
    ...
    map_sub_ = create_subscription<sensor_msgs::msg::PointCloud2>(
      ...
      std::bind(&FastPlanningNode::onMap, this, std::placeholders::_1));
    odom_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      get_parameter("OdomTopic").as_string(), 50,
      std::bind(&FastPlanningNode::onOdom, this, std::placeholders::_1));
    goal_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>(
      get_parameter("TargetTopic").as_string(), 10,
      std::bind(&FastPlanningNode::onGoal, this, std::placeholders::_1));
    timer_ = create_wall_timer(std::chrono::milliseconds(20),
                               std::bind(&FastPlanningNode::onTick, this));

    RCLCPP_INFO(get_logger(),
      "Path F Fast-Planner-adapter ready: map=%s → /planner/*", map_topic_.c_str());
  }
...
private:
  ...
  void plan() {
    ...
    if (waypoints.size() < 2) {
      RCLCPP_WARN(get_logger(), "Path F A* failed");
      return;
    }
    ...
    RCLCPP_INFO(get_logger(), "Path F planned %zu Hermite samples", traj_.size());
  }
...
int main(int argc, char **argv) {
  ...
  rclcpp::spin(std::make_shared<FastPlanningNode>());
  rclcpp::shutdown();
  return 0;
}
