#!/usr/bin/env bash
# Fix Path H throughput: fewer GPU updates per env step (restore ~old speed).
set -euo pipefail
...
cd "$WS"
echo "==== speed-fix $(date -Is) batch=128 updates=2 n_envs=2 resume=${RESUME_ARGS[*]:-none} ====" | tee -a "$LOG"
...
disown || true
sleep 8
pgrep -af 'python3 -m drone_rl_planner.train_sac_polar' | grep -v sandbox | head -1
tail -8 "$LOG"
