...
MIGHTY_NODE::MIGHTY_NODE() : Node("mighty_node") {
  // Get id from ns (upstream expects NX01 / RR02 … last two chars = agent id).
  // Solo plant launches use /NX01; root "/" has no digits — default id 1.
  ns_ = this->get_namespace();
  if (!ns_.empty() && ns_.back() == '/') {
    ns_.pop_back();
  }
  auto slash = ns_.find_last_of('/');
  ns_ = (slash == std::string::npos) ? ns_ : ns_.substr(slash + 1);
  if (ns_.size() >= 2) {
