/* Adapted from ego-planner-swarm ros2_version: bspline_opt/uniform_bspline.{h,cpp}
 * Changes: namespace ego_planner -> drone_planner; include path; no so3/fake_drone deps.
 */
#ifndef DRONE_PLANNER_UNIFORM_BSPLINE_HPP
#define DRONE_PLANNER_UNIFORM_BSPLINE_HPP
...
namespace drone_planner
{
  ...
  {
  private:
    // Control points: dimension x N (EGO layout).
    // Each COLUMN is one control point; rows are xyz for 3D.
    // e.g. N points in 3D -> 3xN matrix (see parameterizeToBspline).
