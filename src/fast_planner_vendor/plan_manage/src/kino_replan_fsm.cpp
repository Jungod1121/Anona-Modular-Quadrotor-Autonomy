...
void KinoReplanFSM::init(std::shared_ptr<FastPlanner> nh) {
  ...
  nh->declare_parameter("fsm/waypoint3_x", -1.0);
  nh->declare_parameter("fsm/waypoint3_y", -1.0);
  nh->declare_parameter("fsm/waypoint3_z", -1.0);
  nh->declare_parameter("fsm/waypoint4_x", -1.0);
  nh->declare_parameter("fsm/waypoint4_y", -1.0);
  nh->declare_parameter("fsm/waypoint4_z", -1.0);
  nh->declare_parameter("fsm/waypoint5_x", -1.0);
  nh->declare_parameter("fsm/waypoint5_y", -1.0);
  nh->declare_parameter("fsm/waypoint5_z", -1.0);
  nh->declare_parameter("fsm/waypoint6_x", -1.0);
  nh->declare_parameter("fsm/waypoint6_y", -1.0);
  nh->declare_parameter("fsm/waypoint6_z", -1.0);
...
void KinoReplanFSM::waypointCallback(const nav_msgs::msg::Path::SharedPtr msg) {
  ...
  if (target_type_ == TARGET_TYPE::MANUAL_TARGET) {
    ...
    // One trigger starts the chain at waypoint 0; later doors auto-advance on reach.
    current_wp_ = 0;
  ...
  switch (exec_state_) {
    ...
    case GEN_NEW_TRAJ:{
      ...
      case EXEC_TRAJ: {
            ...
            /* Local B-spline finished. Upstream clears the target; with a
             * slower plant tracker that can happen before we actually arrive.
             * Keep chasing via odom unless we are already near the goal. */
            if (t_cur > info->duration_ - 1e-2) {
              const double dist_to_goal = (end_pt_ - odom_pos_).norm();
              if (dist_to_goal < no_replan_thresh_) {
                // Narrow S-bend: chain preset door centers without re-clicking goals.
                if (target_type_ == TARGET_TYPE::PRESET_TARGET &&
                    current_wp_ < waypoint_num_) {
                  end_pt_(0) = waypoints_[current_wp_][0];
                  end_pt_(1) = waypoints_[current_wp_][1];
                  end_pt_(2) = waypoints_[current_wp_][2];
                  RCUTILS_LOG_INFO(
                      "Preset advance → wp %d / %d  (%.2f, %.2f, %.2f)",
                      current_wp_ + 1, waypoint_num_, end_pt_(0), end_pt_(1),
                      end_pt_(2));
                  current_wp_++;
                  end_vel_.setZero();
                  have_target_ = true;
                  ...
                  std::cout << "Goal reached (odom dist=" << dist_to_goal
                            << "), waiting for next target." << std::endl;
                  have_target_ = false;
                  changeFSMExecState(WAIT_TARGET, "FSM");
                }
              } else {
                std::cout << "Traj finished but plant lagging (odom dist="
                          << dist_to_goal << "), regenerate." << std::endl;
                changeFSMExecState(GEN_NEW_TRAJ, "FSM");
              }
              return;

            } else if ((end_pt_ - pos).norm() < no_replan_thresh_) {
              // Near planned end — hold, wait for duration / odom check above.
              return;
    ...
    case REPLAN_TRAJ:{
      ...
      if (start_pt_(2) < 0.4 || start_pt_(2) > 2.4 ||
          (start_pt_ - odom_pos_).norm() > 2.0) {
        start_pt_ = odom_pos_;
        start_vel_ = odom_vel_;
        start_acc_.setZero();
      }
