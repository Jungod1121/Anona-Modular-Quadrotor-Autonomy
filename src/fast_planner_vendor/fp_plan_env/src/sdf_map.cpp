...
void SDFMap::initMap(std::shared_ptr<FastPlanner> node) {
  ...
  node->declare_parameter<int>("sdf_map/skip_pixel", 2);
  ...
  node->declare_parameter<bool>("sdf_map/show_esdf_time", false);
  ...
  // Initialize other parameters
    ...
    if (mp_.skip_pixel_ < 1) mp_.skip_pixel_ = 1;
    ...
    if (mp_.pose_type_ == POSE_STAMPED) {
      ...
      sync_image_pose_->registerCallback(std::bind(&SDFMap::depthPoseCallback, this, std::placeholders::_1,std::placeholders::_2 ));
        ...
        odom_sub_->subscribe(node.get(), "/sdf_map/odom");
    ...
    // TRANSIENT_LOCAL matches latched /map_generator/global_cloud (same as EGO grid_map).
    // Default volatile QoS never receives the latched map → empty SDF → fly through walls.
...
void SDFMap::clearAndInflateLocalMap() {
  ...
  // Fill ceil_id..map_top (not a single slab). Kino primitives with tau~0.6 and
  // v~0.7 travel ~0.4 m/step and can jump a 0.15 m layer, then climb forever.
  if (mp_.virtual_ceil_height_ > -0.5) {
    int ceil_id = floor((mp_.virtual_ceil_height_ - mp_.map_origin_(2)) * mp_.resolution_inv_);
    int z_max = std::min(md_.local_bound_max_(2), mp_.map_voxel_num_(2) - 1);
    if (ceil_id >= 0) {
      for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
        for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
...
void SDFMap::odomCallback(const nav_msgs::msg::Odometry::ConstSharedPtr odom) {
    ...
    const bool first_odom = !md_.has_odom_;
    md_.has_odom_ = true;

    // Latched cloud often arrives before odom — apply cached cloud once pose is ready.
    if (first_odom && has_cached_cloud_ && !cached_cloud_.points.empty()) {
      processCloud(cached_cloud_);
    }
}
...
void SDFMap::cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr img) {
    ...
    // Always keep last global cloud so we can ingest after odom appears.
    cached_cloud_ = latest_cloud;
    ...
    processCloud(latest_cloud);
}

void SDFMap::processCloud(const pcl::PointCloud<pcl::PointXYZ> & latest_cloud) {
    if (latest_cloud.points.empty()) return;

    if (std::isnan(md_.camera_pos_(0)) || std::isnan(md_.camera_pos_(1)) || std::isnan(md_.camera_pos_(2))) return;

    // After the one-shot global seed, only slide the local ESDF/viz window with
    // the drone. Do NOT resetBuffer — that would wipe distant gate walls and
    // force a perpetual full-map red "perception" box.
    if (global_map_seeded_) {
    ...
    // First ingest: load the ENTIRE latched map into occupancy + one full ESDF.
    ...
    if (mp_.virtual_ceil_height_ > -0.5) {
      int ceil_id =
          floor((mp_.virtual_ceil_height_ - mp_.map_origin_(2)) * mp_.resolution_inv_);
      int z_max = std::min(md_.local_bound_max_(2), mp_.map_voxel_num_(2) - 1);
      if (ceil_id >= 0) {
        for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
          for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
    ...
    // Virtual floor — without this, B-spline / kino dive and REPLAN starts at z<0.
    {
      const double floor_z = std::max(0.05, mp_.ground_height_ + 0.08);
      int floor_id = static_cast<int>(
          floor((floor_z - mp_.map_origin_(2)) * mp_.resolution_inv_));
      if (floor_id < 0) {
        floor_id = 0;
      }
      int z_lo = md_.local_bound_min_(2);
      for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
        for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
          for (int z = z_lo; z <= floor_id; ++z) {
            if (!isInMap(Eigen::Vector3i(x, y, z))) continue;
            md_.occupancy_buffer_inflate_[toAddress(x, y, z)] = 1;
          }
    }

    // One-shot full-map ESDF so distant gates keep valid distances after we
    // shrink the viz/update window (Fast-Planner's normal local mode).
    updateESDF3d();
    md_.esdf_need_update_ = false;
    global_map_seeded_ = true;
    ...
    RCLCPP_INFO(node_->get_logger(),
                "SDFMap: seeded full global cloud (%zu pts); local viz/ESDF "
                "window ±(%.1f,%.1f,%.1f) m",
                latest_cloud.points.size(), mp_.local_update_range_(0),
                mp_.local_update_range_(1), mp_.local_update_range_(2));
}
