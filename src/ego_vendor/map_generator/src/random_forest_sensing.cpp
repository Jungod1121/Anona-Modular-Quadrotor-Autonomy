...
double _min_dist;
double _clear_y = 0.0;  // keep |y| < clear_y free (0 = disabled)
...
void RandomMapGenerateCylinder() {
  ...
  for (int i = 0; i < _obs_num && rclcpp::ok(); i++) {
    ...
    // Clear east-west corridor so start↔goal A* always has free space.
    if (_clear_y > 1e-3 && std::abs(y) < _clear_y) {
      i--;
      continue;
    }
...
int main(int argc, char **argv)
{
    ...
    _all_map_pub = node->create_publisher<sensor_msgs::msg::PointCloud2>("/map_generator/global_cloud", map_qos);
    ...
    node->declare_parameter("ObstacleShape/seed", 0);  // 0 → random_device
    ...
    node->declare_parameter("map/clear_y", 0.0);
    ...
    node->get_parameter("map/clear_y", _clear_y);
