...
> 归档自 `report/planner_benchmark/comparison_report.md`，供正式报告正文引用（见 `report/appendix/README.md`）。逐试次轨迹图仍指向原 `report/planner_benchmark/charts/trials/`。
...
![overview](figures/batch_overview.png)

![success and safety](figures/batch_heatmap.png)

## Per-trial trajectory charts

One figure per planner×map trial (obstacles overlaid when `obstacles.csv` is present).

![Path A — Grid A* + B-spline × Random Forest](../planner_benchmark/charts/trials/homemade__official_forest.png)

![Path A — Grid A* + B-spline × Dense Obstacle Field](../planner_benchmark/charts/trials/homemade__dense_field.png)

![Path B — EGO rebound B-spline × Random Forest](../planner_benchmark/charts/trials/ego__official_forest.png)

![Path B — EGO rebound B-spline × Dense Obstacle Field](../planner_benchmark/charts/trials/ego__dense_field.png)

![Path C — GCOPTER / MINCO × Random Forest](../planner_benchmark/charts/trials/gcopter__official_forest.png)

![Path C — GCOPTER / MINCO × Dense Obstacle Field](../planner_benchmark/charts/trials/gcopter__dense_field.png)

![Path E — MIGHTY HGP × Random Forest](../planner_benchmark/charts/trials/mighty__official_forest.png)

![Path E — MIGHTY HGP × Dense Obstacle Field](../planner_benchmark/charts/trials/mighty__dense_field.png)

![Optional F — Fast-Planner kino (lineage) × Random Forest](../planner_benchmark/charts/trials/fast_planner__official_forest.png)

![Optional F — Fast-Planner kino (lineage) × Dense Obstacle Field](../planner_benchmark/charts/trials/fast_planner__dense_field.png)

![Path G — VFH+ histogram × Random Forest](../planner_benchmark/charts/trials/vfh__official_forest.png)

![Path G — VFH+ histogram × Dense Obstacle Field](../planner_benchmark/charts/trials/vfh__dense_field.png)

![Path H — Polar DrQ-SAC × Random Forest](../planner_benchmark/charts/trials/sac__official_forest.png)

![Path H — Polar DrQ-SAC × Dense Obstacle Field](../planner_benchmark/charts/trials/sac__dense_field.png)
...
- `figures/batch_overview.png` / `figures/batch_heatmap.png`: aggregate summary only
- `../planner_benchmark/charts/trials/<planner>__<map>.png`: one trajectory figure per trial
