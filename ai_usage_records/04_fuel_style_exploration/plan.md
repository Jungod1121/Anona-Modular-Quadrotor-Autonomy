...
todos:
  - id: clone-fuel-readonly
    content: Clone FUEL into reference_repos with COLCON_IGNORE; note what we borrow vs never ship
    status: completed
  - id: pkg-drone-exploration
    content: "Add drone_exploration: local sensing + frontier + exploration FSM publishing /drone/goal"
    status: completed
  - id: launch-path-d
    content: Add fuel_explore.launch.py + planner_sim/dashboard Path D wiring (plant unchanged)
    status: completed
  - id: smoke-demo
    content: "Demo: trigger explore, verify goals+trajectory; sanity-check Path A still launches"
    status: completed
isProject: false
---
