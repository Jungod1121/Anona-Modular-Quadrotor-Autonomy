...
todos:
  - id: launch-switch
    content: Add planner_sim.launch.py with planner:=homemade|ego|gcopter; keep old launches
    status: completed
  - id: bridge-contract
    content: "Document/enforce unified outputs: local_goal + trajectory_cmd (+ Path)"
    status: completed
  - id: vendor-gcopter
    content: Vendor ROS2 GCOPTER/MINCO (no SO3 plant); build under src/gcopter_vendor
    status: completed
  - id: gcopter-launch-bridge
    content: gcopter_avoidance.launch + bridge to our controller; official map_generator
    status: completed
  - id: readme-multi
    content: "README: three launch commands and planner comparison note"
    status: completed
isProject: false
---
